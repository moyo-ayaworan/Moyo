...
function RecordPayment({ doc, headers, pendingRequests, onUpdated, onClose }: { doc: ManagedDocument; headers: Record<string, string>; pendingRequests: Map<number, string>; onUpdated: (doc: ManagedDocument) => void; onClose: () => void }) {
  ...
  return <div className="mt-4 space-y-4 rounded-lg border border-white/20 bg-black/20 p-4">
    ...
    <div className="flex gap-3"><button type="button" className={button} disabled={busy || !confirmed} onClick={save}>{busy ? 'Saving…' : uncertain ? 'Retry same payment' : 'Confirm received & create receipt'}</button><button type="button" className={button} disabled={busy || uncertain} onClick={onClose}>Cancel</button></div>
  </div>;
}
...
export default function DocumentManager({ gallery, galleries = [], documents, actions, headers, onSend, onDownload, onDelete, onUpdated, onNew, children }: {
  ...
  const controls = (doc: ManagedDocument) => <div className="mt-4 flex flex-wrap gap-2"><button type="button" className={button} disabled={Boolean(actions[`send-${doc.id}`])} onClick={() => onSend(doc)}>{actions[`send-${doc.id}`] ? 'Sending…' : `Email ${doc.viewKind || 'contract'}`}</button><button type="button" className={button} disabled={Boolean(actions[`download-${doc.id}`])} onClick={() => onDownload(doc)}>{actions[`download-${doc.id}`] ? 'Downloading…' : `Download ${doc.viewKind || 'contract'}`}</button></div>;
